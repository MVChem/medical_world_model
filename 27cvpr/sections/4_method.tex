...
\paragraph{Eight-slot multimodal state.}
We encode each observation as four vision--language slots and four visual slots (\cref{fig:task_supervised_slots}). The vision--language branch fuses adapted frozen JEPA features with the available report; the visual branch uses the VLM's pre-fusion vision features. Learned readouts extract one slot from each of four depths per branch:
\begin{equation}
    \begin{aligned}
        s_{\tau,j}^{\mathrm{f}} &= r_j^{\mathrm{f}}(H_\tau^{(\ell_j)}),\quad
        s_{\tau,j}^{\mathrm{v}} = r_j^{\mathrm{v}}(F_\tau^{(k_j)}),\\
        S_\tau &= E_\eta(O_\tau)=[S_\tau^{\mathrm{f}};S_\tau^{\mathrm{v}}]
        \in\mathbb R^{8\times d},
    \end{aligned}
    \label{eq:vlm_state}
\end{equation}
where $j=1,\ldots,4$ and $d=1024$. The selected depths span early, intermediate, and final layers. All eight slots support task decoding and temporal prediction; state construction is independent of the task query and forecast horizon.

\paragraph{Visual Slot Spatial Consistency (VSSC).}
To preserve spatial information in the four visual slots, an auxiliary decoder reconstructs a dense feature field solely from $S_\tau^{\mathrm{v}}$. We align this field with frozen native-vision teacher features from the original image and two independently encoded crops. Matching spatial transformations and channel normalization precede a squared-error loss over valid regions, denoted $\mathcal L_{\mathrm{VSSC}}$. This training-only objective supervises the visual-slot encoder during segmentation updates.
