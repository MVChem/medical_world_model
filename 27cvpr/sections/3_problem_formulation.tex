...
An encoder $E_\eta$ maps each observation to multimodal state tokens $S_\tau\in\mathbb R^{K\times d}$. All three current tasks additionally receive native image tokens from $X_\tau$ through a shared task decoder. Given $S_t$ and a requested temporal horizon $h$, a latent world model predicts the follow-up state:
\begin{equation}
    \begin{gathered}
