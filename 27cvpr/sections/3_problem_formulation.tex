...
An encoder $E_\eta$ maps each observation to multimodal state tokens $S_\tau\in\mathbb R^{K\times d}$. Tasks requiring spatial detail additionally receive the input image $X_\tau$ directly. For report forecasting, the native vision--language decoder retains the current image and admissible text, augmented with the predicted state. Given $S_t$ and a requested temporal horizon $h$, a latent world model predicts the follow-up state:
\begin{equation}
    \begin{gathered}
...
\end{equation}
The target $S^*_{t^+}$ is encoded from the actual follow-up. We distinguish the requested horizon $h$ from the realized follow-up time $t^+$; their association is set by the temporal sampling protocol. The retrospective forecasting protocol assumes the current report is available at the source observation; other clinical records must satisfy the source-time cutoff. Follow-up images and text supply detached state targets, supervised report/finding targets, and scoring references; they are unavailable as evidence to the predictor or forecast decoder.
