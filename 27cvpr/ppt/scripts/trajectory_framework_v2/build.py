...
def rounded_commands(points, radius=.065, corner_curve=1):
    """Rounded polygon; .5522848 gives circular quarter-arc tangents."""
    commands = []
    for i, point in enumerate(points):
        ...
        c1 = tuple(a + corner_curve*(p-a) for a, p in zip(before, point))
        c2 = tuple(a + corner_curve*(p-a) for a, p in zip(after, point))
        commands.append(("C", *c1, *c2, *after))
    commands.append(("Z",))
    return commands


def rounded_quad(parent, name, points, radius=0.065, corner_curve=1, **kwargs):
    """Round all four corners of a perspective card with cubic segments."""
    return path(parent, name, rounded_commands(points, radius, corner_curve), **kwargs)
...
def panel_one(slide):
    ...
    # Trace the reference's individual perspective planes. Broad glass faces
    # mostly occlude the planes behind them; distance is conveyed by lighter
    # colors, rather than accumulating several equally transparent blue fills.
    # Values are (corners, left blue, right blue, face opacity, lower veil).
    cards = [
        ([(457, 92), (621, 120), (621, 259), (457, 220)], "E1EDFD", "DEEAFC", .88, .14),
        ([(393, 115), (563, 144), (563, 281), (393, 241)], "DAE8FD", "D4E6FD", .90, .21),
        ([(333, 137), (489, 164), (489, 299), (333, 260)], "C8DEFD", "C1DAFC", .92, .34),
        ([(271, 159), (417, 186), (417, 339), (271, 301)], "BCD7FC", "B4D2FC", .93, .42),
        ([(211, 182), (359, 209), (359, 365), (211, 325)], "AED0FB", "A5CAFB", .93, .47),
        ([(150, 204), (301, 233), (301, 391), (150, 348)], "9DC8FA", "96BFF8", .93, .48),
        ([(90, 227), (241, 259), (241, 419), (90, 376)], "8FBFF8", "80B5F7", .92, .12),
    ]
    for i, (points, left, right, opacity, veil) in enumerate(cards):
        card = group(panel, f"01: frame {7-i:02} (rear to front)")
        if i == 2:
            # Two faint displaced lower edges are visible in the reference
            # behind the middle cards. Their tops are occluded by the faces.
            for label, corners, alpha in [
                ("rear", [(368, 258), (481, 283), (481, 315), (368, 288)], .22),
                ("near", [(368, 269), (464, 292), (464, 320), (368, 296)], .38),
            ]:
                reflection = rounded_quad(card, f"Faint depth reflection: {label}", corners,
                                          radius=.13, corner_curve=.5522848, fill="A0C8F4")
                gradient(reflection, [(0, "A0C8F4", alpha*.6), (1, "A0C8F4", alpha)], 90)
        for width, alpha in [(6, .025), (3.5, .045)]:
            edge = rounded_quad(card, "Soft glass edge", points, radius=.105,
                                corner_curve=.5522848,
                                fill=None, line=right, width=width)
            edge.line._get_or_add_ln().solidFill[0].append(
                xml("a:alpha", val=round(alpha*100000)))
        shape = rounded_quad(card, "Glass face: depth color", points, radius=.105,
                             corner_curve=.5522848, fill=left)
        gradient(shape, [(0, left, opacity), (1, right, opacity)], 0)
        wash = rounded_quad(card, "Glass face: lower fade", points, radius=.105,
                            corner_curve=.5522848, fill="FFFFFF")
        gradient(wash, [(0, "FFFFFF", 0), (.30, "FFFFFF", 0),
                        (.75, "FFFFFF", veil*.55), (1, "FFFFFF", veil)], 90)
        if i == 6:
            inner = [(96, 234), (235, 264), (235, 412), (96, 371)]
            ring = rounded_commands(points, .105, .5522848)
            ring += rounded_commands(list(reversed(inner)), .085, .5522848)
            frame = path(card, "Front image border: fades toward bottom", ring, fill="609BEF")
            gradient(frame, [(0, "609BEF", 1), (.4, "6CA3F1", .90),
                             (.78, "92BDF4", .30), (1, "A9CDF6", .02)], 90)
            # The front plane is the only fully visible landscape thumbnail.
            mountain = path(card, "Front image mountains", [
                ("M", 103, 351), ("L", 135, 300),
                ("C", 138, 295, 143, 295, 146, 301),
                ("L", 175, 348), ("L", 183, 331),
                ("C", 187, 325, 192, 325, 196, 333),
                ("L", 216, 371), ("Z",)], fill="5195E8")
            gradient(mountain, [(0, "4E94E9", .79), (.7, "609EED", .80),
                                (1, "8CB8F0", .66)], 90)
    ...
    # The reference has a white luminous band crossing all glass cards. Build
    # this with translucent native strokes, so it is editable and pixel-free.
    glow = group(trajectory, "Trajectory soft white light")
    previous = 0
    for j in range(50):
        width = 90 - j*1.5
        target = .80*math.exp(-.5*(width/2/14)**2)
        alpha = (target-previous)/(1-previous)
        previous = target
        stroke = path(glow, f"White light: width {width}", curve, line="FFFFFF", width=width)
        stroke.line._get_or_add_ln().solidFill[0].append(
            xml("a:alpha", val=round(alpha*100000)))
    path(trajectory, "Trajectory white rim", curve, line="FFFFFF", width=15)
    path(trajectory, "Trajectory blue Bezier", curve, line="2582FA", width=11)
    head = rounded_quad(trajectory, "Trajectory arrow head", [(550, 165), (594, 168), (567, 202)],
                        radius=.08, fill="2582FA")
    for i, (cx, cy) in enumerate([(239, 345), (352, 291), (456, 235)]):
        for radius, alpha in [(23, .07), (20, .12), (18, .18)]:
            oval(trajectory, f"Time point {i+1}: soft light", cx-radius, cy-radius,
                 radius*2, radius*2, "FFFFFF", opacity=alpha)
