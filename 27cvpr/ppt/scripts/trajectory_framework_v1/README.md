...
- `../../ppt/trajectory_framework_v1.pptx`：单页可编辑 PPT，页面比例为 3:1。
- `../../ppt/trajectory_framework_v1.png`：由最终 PPT 经 LibreOffice 渲染的 3072 × 1024 预览。
- `build.py`：重建和渲染入口。
- `assets/reference.png`：用户原始透明 PNG 的副本，2172 × 724；仅用于对照，不嵌入 PPT。

## 编辑能力

幻灯片包含 **110 个原生形状和文本框**，其中有 34 个采用三次 Bézier 曲线的自由形状。没有嵌入 SVG 或位图。三个面板分别组成命名分组；在 PowerPoint 中可用“选择窗格”、进入分组或取消组合来选中卡片、图标、环形箭头、时间点、同心圆和标题。
...
脚本以自身路径查找输出位置，不依赖工作目录或机器上的固定 Python 路径。移动文件时保持 `ppt/` 与 `scripts/trajectory_framework_v1/` 的相对关系。首次生成不需要 `--overwrite`。
