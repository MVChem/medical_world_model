"""Clin-JEPA w/o warmup ablation (paper §5.2): removes Phase 1 (predictor warmup).

The phase state machine enters at Phase 2a with the encoder trainable from
step 0; Phase 1 is configured to zero steps. The Phase 4 hard sync is kept.
"""
...
RESUME_SCHEMA_VERSION = "ablation_no_warmup_v1"
...
def compute_rollout_loss_native(
    ...
    """Native autoregressive-chaining rollout loss (single-space regime)."""
...
def compute_rollout_loss_teacher_forced(
    predictor,
    z_states_online: torch.Tensor,
    z_actions_online: torch.Tensor,
    z_states_target: torch.Tensor,
...
def phase_1_step(raw_online, predictor, tokenizer, batch_windows, device,
    ...
    """Phase 1: encoder frozen, predictor warmup. Single-space (online == target)."""
...
def phase_2a_step(raw_online, predictor, tokenizer, batch_windows, device,
                  max_seq_len, rollout_horizon):
    """Phase 2a: joint co-training. Mixed-space (online ≠ target). Teacher-forced rollout."""
    raw_online.set_adapter("online")
...
def phase_2b_step(raw_online, predictor, tokenizer, batch_windows, device,
                  max_seq_len, rollout_horizon):
    """Phase 2b: online frozen, target EMA chases. Mixed-space, teacher-forced."""
    raw_online.set_adapter("online")
    with torch.no_grad():
        enc_online = encode_batch_via_online(
            raw_online, tokenizer, batch_windows, device, max_seq_len,
...
def phase_4_step(raw_online, predictor, tokenizer, batch_windows, device,
                 max_seq_len, rollout_horizon):
    """Phase 4: encoder fully frozen post-sync, predictor finalize. Single-space."""
    raw_online.set_adapter("online")
    with torch.no_grad():
        enc = encode_batch_via_online(
            raw_online, tokenizer, batch_windows, device, max_seq_len,
...
def evaluate_ablation(
    ...
    """Val teacher-forcing loss + z_std collapse monitor.
    use_target=True for Phase 2a/2b (compare to target); False for Phase 4.
    """
    predictor.eval()
    n_windows = 0
    total_loss = 0.0
...
def save_ablation_checkpoint(
    ...
    """PEFT save layout same as Clin-JEPA: out/online/ + out/predictor.pt + out/meta.json."""
...
def save_resume_state_ablation(
    ...
    """Save ablation training state (ablation schema_version)."""
    state = {
...
def load_resume_state_ablation(
    ...
    """Load resume.pt and restore mutable training state. Schema-version checked."""
...
def train(config: dict, args: argparse.Namespace) -> None:
    ...
    if is_main:
        setup_logging()
        logger.info(
            "Clin-JEPA w/o warmup — phase_1 removed (%d GPUs)", world_size,
        )
    ...
    if args.smoke:
        phase_steps = {
            "phase_1": 0,  # no-warmup ablation: phase_1 has zero steps
            "phase_2a": args.smoke_phase_steps if args.smoke_phase_steps else 5,
            "phase_2b": args.smoke_phase_steps if args.smoke_phase_steps else 3,
    ...
    if is_main:
        logger.info(
            "phase schedule: 1=%d, 2a=%d, 2b=%d, 3=instant_sync, 4=%d, total=%d",
            phase_steps["phase_1"], phase_steps["phase_2a"],
            phase_steps["phase_2b"], phase_steps["phase_4"], total_steps,
        )
        if phase_steps["phase_1"] != 0:
            logger.warning(
                "This script expects phase_1_steps=0 to test the no-warmup "
                "hypothesis, but config has phase_1_steps=%d. "
    ...
    # z_std collapse early-abort threshold (default 0.05).
    z_std_abort_threshold = log_cfg.get("z_std_abort_threshold", 0.05)
    ...
    # ====================================================================
    # No-warmup entry point — start at phase_2a with the encoder trainable
    # ====================================================================
    set_encoder_trainable(online, True)
    current_phase = "phase_2a"
    if is_main:
        logger.info(
            "=== ENTERING PHASE 2a DIRECTLY (no warmup, "
            "encoder trainable from step 0) ==="
        )
    ...
    if resume_path is not None and resume_path.exists():
        ...
        # Restore encoder-trainable state for current phase
    ...
    while global_step < total_steps:
        ...
        if micro_step % grad_accum == 0:
            ...
            # ====================================================================
            # Phase transitions gated on phase_steps[X] > 0 so phase_1_steps = 0
            # ====================================================================
            new_phase = current_phase
            if global_step == boundary_phase_2a and phase_steps["phase_2a"] > 0 \
                    and phase_steps["phase_1"] > 0:
                # dead branch when phase_1_steps == 0
                new_phase = "phase_2a"
            ...
            if global_step % eval_every == 0:
                ...
                if is_main:
                    ...
                    if not eval_failed and val_metrics is not None:
                        ...
                        if z_std < z_std_abort_threshold and global_step > 100:
                            logger.warning(
                                "z_std=%.4f below abort threshold %.4f — encoder "
                                "representation collapsed; aborting and saving "
    ...
    if is_main:
        ...
        logger.info(
            "Clin-JEPA w/o warmup complete. Best val: %.4f, %d steps, %.1f min",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Clin-JEPA w/o warmup: phase_1 (predictor warmup) removed",
    )
    parser.add_argument("--config", required=True,
                        help="Path to configs/train/ablation_no_warmup.yaml")
