...
def main(args, resume_preempt=False):
    ...
    init_checkpoint_key = cfgs_meta.get("init_checkpoint_key", "ema_encoder")
    seed = cfgs_meta.get("seed", _GLOBAL_SEED)
    checkpoint_freq = cfgs_meta.get("checkpoint_freq", CHECKPOINT_FREQ)
    save_every_freq = cfgs_meta.get("save_every_freq", -1)
    save_checkpoints = cfgs_meta.get("save_checkpoint", True)
    ...
    dataset_paths = [
        ...
        for path in cfgs_data.get("datasets", [])
    ]
    ...
    if cfgs_img_data is not None:
        ...
        img_dataset_paths = [
            os.path.expandvars(os.path.expanduser(path))
            for path in cfgs_img_data.get("datasets", [])
        ]
    ...
    img_rank_weight = cfgs_loss.get("img_rank_weight", 1.0)
    vid_rank_weight = cfgs_loss.get("vid_rank_weight", 1.0)
    ...
    if cfgs_img_data is not None:
        ...
        if rank < int(world_size * img_rank_ratio):
            ...
            rank_loss_weight = img_rank_weight
        else:
            ...
            rank_loss_weight = vid_rank_weight
    ...
    if init_checkpoint is not None:
        init_checkpoint = os.path.expandvars(os.path.expanduser(init_checkpoint))
        checkpoint = torch.load(
            init_checkpoint, map_location="cpu", mmap=True, weights_only=True
        )
        pretrained = {
            key.removeprefix("module."): value
            for key, value in checkpoint[init_checkpoint_key].items()
        }
        encoder.load_state_dict(pretrained, strict=True)
        logger.info(
            "Initialized encoder from %s[%s]", init_checkpoint, init_checkpoint_key
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if save_checkpoints and (
            (epoch + 1) % checkpoint_freq == 0 or epoch == (num_epochs - 1)
        ):
