...
async def run(url, output):
    ...
    async with async_playwright() as pw:
        ...
        # Requesting all tables repeatedly must reuse ready/in-flight patient reads.
        response = await page.request.post(
            url + "/api/patients/12137189/tables", data={"tables": ["all"]}
        )
        assert response.ok
        for _ in range(240):
            manifest = await (
        ...
        checks["checks"].append(
            "All 17 tables read from persistent patient index; counts known before loading; zero source CSV rows scanned"
        )
