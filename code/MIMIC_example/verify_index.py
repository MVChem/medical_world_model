...
from .patient_index import PatientIndex, connect
...
def verify(cache):
    ...
    for name, meta in cache.tables.items():
        ...
        files = {p.name: pq.ParquetFile(p) for p in folder.glob("*.parquet")}
        rows = sum(p.metadata.num_rows for p in files.values())
        assert rows == meta["rows"], (name, rows, meta["rows"])
        for filename, parquet in files.items():
            assert parquet.schema_arrow.names == [
                *meta["fields"],
                "__mimic_source_row",
            ], (name, filename)
            for group in range(parquet.num_row_groups - 1):
                assert (
                    parquet.metadata.row_group(group).num_rows == meta["row_group_size"]
                )
        if meta["patient_table"]:
            offsets = Counter()
            patients = 0
            with closing(connect(folder / "subjects.sqlite")) as db:
                for subject, filename, start, count in db.execute(
                    "SELECT subject_id,filename,start_row,row_count FROM subjects ORDER BY filename,start_row"
                ):
                    assert subject and count > 0 and start == offsets[filename], (
                        name,
                        subject,
                    )
            ...
            assert dict(offsets) == {f: p.metadata.num_rows for f, p in files.items()}
    ...
    with closing(connect(cache.root / "cxr.sqlite")) as db:
        ...
        for subject in ("10000032", "10004606", "12137189"):
            for payload, report, status in db.execute(
                "SELECT payload,report,report_status FROM studies WHERE subject_id=?",
                (subject,),
            ):
                path = (
                    Path(cache.manifest["cxr_root"])
                    / json.loads(payload)["report_relative_path"]
                )
                assert (
                    (path.read_text() == report)
                    if status != "missing"
...
def verify_api(cache, url):
    ...
    for subject in ("10004606", "12137189", "10000032"):
        ...
        request(f"/api/patients/{subject}/tables", {"tables": names})
        for _ in range(600):
        ...
        output[subject] = {
            ...
            "four_tables_seconds": round(load_seconds, 3),
