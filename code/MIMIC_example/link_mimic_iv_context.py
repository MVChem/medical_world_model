...
def resolve_mimic_iv_root(path: Path) -> Path:
    ...
    for candidate in candidates:
        if (candidate / "hosp/admissions.csv.gz").is_file() and (
            candidate / "icu/icustays.csv.gz"
        ).is_file():
            return candidate
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--transitions",
        type=Path,
        default=Path("example_output/transitions.jsonl"),
        help="CXR transition packets produced by build_mimic_transitions.py",
    )
    parser.add_argument("--output-dir", type=Path, default=Path("linked_output"))
    parser.add_argument(
        "--include-icu-inputs",
        action="store_true",
