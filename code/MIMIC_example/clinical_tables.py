...
class ClinicalTables:
    ...
    def request(self, subjects, names):
        ...
        with self.lock:
            for name in names:
                ...
                future = self.executor.submit(self._load, spec, pending, path)
                for subject in pending:
                    self.futures[subject, name] = future
    ...
    def _load(self, spec, subjects, path):
        ...
        try:
            ...
            if self.index:
                ...
                selected = {s: self.index.read_subject(table_name, s) for s in subjects}
                fields, scanned = self.index.table(table_name)["fields"], 0
            else:
                selected, fields, scanned = scan_subjects(
                    path, subjects, progress, self.stop
                )
            for subject in subjects:
            ...
            LOG.info(
                "Loaded %s: scanned %s rows, retained %s rows for %s subjects",
                name,
                scanned,
                sum(len(selected[s]) for s in subjects),
                len(subjects),
            )
        except Exception as exc:
            LOG.exception("Clinical table scan failed: %s", name)
            with self.lock:
                for subject in subjects:
