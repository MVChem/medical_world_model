"""Summarize final joint-model downstream tests against audited native Qwen 0.8B."""
import argparse
import json
...
def compare(run):
    ...
    rows.append({'model':'MedWorld joint','classification_auroc':metrics['classification']['macro_auroc'],
    ...
    result={'rows':rows,'deltas_vs_native':{row['model']:{k:row[k]-rows[0][k] for k in fields if row[k] is not None and rows[0][k] is not None} for row in rows[1:]},
            ...
            'test_policy':'Fixed held-out test; final checkpoint, no test-based selection.'}
    ...
    temporal_root=run/'evaluation/temporal'
    ...
    result['temporal_classification']={
        'n':len(temporal_rows),'native':native_temporal,'joint':trained_temporal,
    ...
    lines+=['','Native Qwen uses original Yes/No likelihoods; adapted models use trained classification heads.',
            ...
            'Joint temporal evaluation is stored separately in `evaluation/temporal/`.','',
            '| Temporal prediction | AUROC | AP |','|---|---:|---:|',
            f"| Native Qwen | {native_temporal['macro_auroc']:.5f} | {native_temporal['macro_ap']:.5f} |",
            f"| MedWorld joint | {trained_temporal['macro_auroc']:.5f} | {trained_temporal['macro_ap']:.5f} |"]
