...
GPU_ORDER = (1, 2, 3, 6, 7, 0)


def validate_gpu_order(order):
    cards = [int(value) for value in order.split(',')]
    if not cards or len(cards) != len(set(cards)) or any(card not in GPU_ORDER for card in cards):
        raise ValueError('GPU order must contain unique allowed IDs: 1,2,3,6,7,0; GPUs 4 and 5 are forbidden')
    return cards
...
def validate_plan(plan):
    ...
    for job in jobs:
        ...
        if int(job.get('gpus', 1)) not in range(0, 7):
            raise ValueError('GPU request must be 0..6')
    visited = set()
...
def worker(args):
    ...
    try:
        if any(card['index'] not in GPU_ORDER for card in record['cards']):
            raise ValueError('Worker requested a forbidden GPU; GPUs 4 and 5 must never be used')
        if time.time() >= soft:
        ...
        env['CUDA_VISIBLE_DEVICES'] = ','.join(str(c['index']) for c in record['cards'])
...
def run(args):
    gpu_order = validate_gpu_order(args.gpu_order)
    os.umask(0o077)
...
def main():
    ...
    p.add_argument('--max-gpus', type=int, choices=range(1, 7), default=6)
    p.add_argument('--gpu-order', default=','.join(map(str, GPU_ORDER)))
    p.add_argument('--soft-deadline', required=True)
