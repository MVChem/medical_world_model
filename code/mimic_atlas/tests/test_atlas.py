...
from mimic_atlas.data import AtlasConfig
from mimic_atlas.setup_data import setup_links
...
def test_uncompressed_iv_sources_after_removing_gzip(sources, tmp_path):
    root, config = sources
    for compressed in config.iv_root.rglob("*.csv.gz"):
        with gzip.open(compressed, "rb") as stream:
            compressed.with_suffix("").write_bytes(stream.read())
        compressed.unlink()
    assert iv.resolve_mimic_iv_root(root) == config.iv_root
    setup_links(root, tmp_path / "plain-data-links")
    tables = iv.load_iv_tables(config.iv_root, {"10000001"}, include_icu_inputs=True)
    assert tables.by_subject["10000001"]["admissions"][0]["hadm_id"] == "20000001"
    assert tables.by_subject["10000001"]["inputevents"][0]["amount"] == "2"
    assert tables.item_dictionary[("1",)]["label"] == "Test input"
    with TestClient(create_app(config)) as client:
        patient = await_patient(client)
        assert patient["clinical_status"]["state"] == "ready"
        assert patient["transitions"][0]["linkage_status"] == "unique_common_admission"
