...
from mimic_atlas import clinical_tables as clinical
...
def test_all_table_api_scans_once_pages_without_loss_and_exports_offline(
    ...
    with (
        ...
        assert 'src="/assets/' not in html.text
        assert 'href="/assets/' not in html.text
...
def test_missing_table_and_retry_after_source_error(tmp_path):
    ...
    try:
        ...
        service.request({"1"}, ["labevents"])
        for _ in range(300):
            if service.manifest("1")[0]["state"] not in {"queued", "loading"}:
                break
            time.sleep(0.01)
        assert (
