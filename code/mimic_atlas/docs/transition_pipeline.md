# Transition generation and explicit exports

The current interactive UI is React + FastAPI (`python -m mimic_atlas` from `code/`). All new CLI export defaults live under `runs/exports/generated_YYYYMMDD/`. The four pre-existing bundles were moved, byte-for-byte, to `runs/exports/legacy_20260918/`; browse and download them in [Export library](http://127.0.0.1:8767/#exports). Source images remain symlinks, and archived summary paths record the original generation location.
...
```bash
cd /home/data2/chk/workspace/2026/08/04/medical_world_model/code/mimic_atlas

/home/data2/chk/workspace/2026/.venv/bin/python build_mimic_transitions.py \
  --mimic-cxr-root ../data/MIMIC \
  --output-dir runs/exports/examples_20260918/cxr \
  --curated-pairs curated_pairs.json \
  --num-examples 4

python -m http.server 8000 --directory runs/exports/examples_20260918/cxr
```

These explicit exports also appear in the React [export library](http://127.0.0.1:8767/#exports). Each case opens the current patient workspace. The standalone legacy gallery may still be opened at <http://localhost:8000>. The default `symlink` asset mode keeps the
...
The argument can also point at `../data/MIMIC`; the builder detects its
...
Run the linker after building `runs/exports/examples_20260918/cxr`:

```bash
/home/data2/chk/workspace/2026/.venv/bin/python link_mimic_iv_context.py \
  --mimic-root ../data/MIMIC \
  --transitions runs/exports/examples_20260918/cxr/transitions.jsonl \
  --output-dir runs/exports/examples_20260918/linked \
  --include-icu-inputs

/home/data2/chk/workspace/2026/.venv/bin/python -m http.server 8000 --directory .
```

Then open <http://localhost:8000/runs/exports/examples_20260918/linked/>. Omit
...
- `runs/exports/examples_20260918/linked/index.html`: current CXR, linked observational timeline, and
  target CXR in three columns.
- `runs/exports/examples_20260918/linked/linked_transitions.jsonl`: full CXR packet plus the uniquely
  resolved admission/stay and traceable clinical context.
- `runs/exports/examples_20260918/linked/appendix_cases.csv`: one compact row per case for plotting or
  assembling a paper figure.
- `runs/exports/examples_20260918/linked/summary.json`: linkage counts, enabled tables, and caveats.
...
/home/data2/chk/workspace/2026/.venv/bin/python build_mimic_transitions.py \
  --mimic-cxr-root ../data/MIMIC \
  --output-dir runs/exports/representative_20260918/cxr \
...
/home/data2/chk/workspace/2026/.venv/bin/python link_mimic_iv_context.py \
  --mimic-root ../data/MIMIC \
  --transitions runs/exports/representative_20260918/cxr/transitions.jsonl \
  --output-dir runs/exports/representative_20260918/linked \
  --include-icu-inputs
```
...
`/runs/exports/representative_20260918/cxr/` or `/runs/exports/representative_20260918/linked/` to inspect
the galleries.
