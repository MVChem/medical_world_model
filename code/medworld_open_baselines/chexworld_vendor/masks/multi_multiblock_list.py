...
class MaskCollator(object):
    ...
    def _sample_block_size(self, generator, scale, aspect_ratio_scale):
        ...
        h = min(self.height, h) # change for efficiency
        w = min(self.width, w)
        # while h >= self.height:
        #     h -= 1
        # while w >= self.width:
    ...
    def __call__(self, batch):
        ...
        all_collated_masks_pred, all_collated_masks_enc = [], []
        ...
        for _ in range(self.nenc):
            ...
            all_collated_masks_enc.append(collated_masks_enc)
            all_collated_masks_pred.append(collated_masks_pred)
        return collated_batch, all_collated_masks_enc, all_collated_masks_pred
