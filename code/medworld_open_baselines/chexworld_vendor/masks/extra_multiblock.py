...
class MaskCollator(object):
    ...
    def __call__(self, batch):
        ...
        for _ in range(B): # different mask for each element
            ...
            masks_e = []
            for _ in range(self.nenc): # generate encode mask first, no unallowed regions
                mask, mask_C = self._sample_block_mask(e_size)
                masks_e.append(mask)
            masks_e = [torch.unique(torch.cat(masks_e))]
            min_keep_pred = min(min_keep_pred, len(masks_p[0]))
            min_keep_enc = min(min_keep_enc, len(masks_e[0]))
