"""One training loop for legacy, 4+4 slots, and full-token Stage 1 readouts."""
...
from bootstrap import ROOT, atomic_json, atomic_torch, digest, seed_all
import torch
from training_variants import build, gradient_record, resolve_variant
from snapshot import source_hashes as collect_source_hashes
...
def optimizer_for(model, cfg):
    ...
    for lora, lr in [(True, cfg["lora_learning_rate"]), (False, cfg["learning_rate"])]:
        params = [
            ...
            if p.requires_grad and ("lora_" in n) == lora
        ]
        if params:
            groups.append(dict(params=params, lr=lr, base_lr=lr))
    return torch.optim.AdamW(groups, betas=(0.9, 0.95), weight_decay=0.01)
...
def should_stop(cfg, variant, follow, step, elapsed, now):
    """No-slot controls match updates, regardless of the reference wall-clock cutoff."""
    if variant == "noslots":
        return step >= cfg["max_steps"]
    if follow:
        return False
    deadline = cfg.get("train_deadline")
    return (
        step >= cfg.get("max_steps", float("inf"))
        or elapsed >= cfg.get("max_train_hours", float("inf")) * 3600
        or (
            deadline is not None and now >= datetime.fromisoformat(deadline).timestamp()
        )
    )
...
def train(args, cfg):
    ...
    if variant == "noslots":
        ...
        if args.freeze_encoder:
            raise ValueError("Baseline trains the same shared parameters as Ours")
    ...
    signatures = {
        n: digest(checkpoint_dir / n)
        for n in [
            "manifest.json",
            "observations.jsonl",
            "features.json",
            "segmentation.json",
            "segmentation_qc.json",
        ]
    }
    if variant != "legacy":
        for key in ["selection", "qa_manifest", "answer_vocabulary"]:
            signatures[key] = digest(Path(cfg[key]))
    signatures["qwen_config"] = digest(Path(cfg["qwen"]) / "config.json")
    signatures["qwen_weights"] = digest(
        Path(cfg["qwen"]) / "model.safetensors-00001-of-00001.safetensors"
    )
    if variant == "noslots":
        signatures["reference_initial_checkpoint"] = digest(
            Path(cfg["reference_initial_checkpoint"])
        )
        reference_signatures = json.loads(
            (args.follow / "provenance.json").read_text()
        )["inputs"]
        assert all(signatures[k] == v for k, v in reference_signatures.items()), (
            "Reference input fingerprints differ"
        )
    ...
    model, Corpus, evaluate = build(cfg, variant, args.freeze_encoder)
    if variant == "noslots":
    ...
    atomic_json(
        ...
        dict(
            ...
            pools={f"{s}/{t}": len(p) for (s, t), p in corpus.pools.items()},
            pos_weight=corpus.pos_weight.tolist(),
            classification_label_coverage=corpus.label_coverage,
            missing_report_by_task=corpus.missing_report,
        ),
    )
    if variant == "noslots" and not args.smoke_steps:
    ...
    try:
        ...
        while not STOP:
            if should_stop(cfg, variant, args.follow, step, elapsed, time.time()):
                break
            if args.follow:
            ...
            for g in optimizer.param_groups:
                g["lr"] = g["base_lr"] * min(1.0, (step + 1) / cfg["warmup_steps"])
            raw = 0.0
            for micro in range(cfg["gradient_accumulation"]):
                ...
                if not torch.isfinite(loss):
                    raise FloatingPointError(f"{task} loss not finite")
                (
                    loss * cfg["loss_weights"][task] / cfg["gradient_accumulation"]
                ).backward()
                raw += float(loss.detach()) / cfg["gradient_accumulation"]
                task_examples[task] += len(indices)
            gradients = gradient_record(model, task, step, args.freeze_encoder, variant)
            grad_norm = float(
            ...
            record = dict(
                ...
                task=task,
                loss=raw,
                weighted_loss=raw * cfg["loss_weights"][task],
                **gradients,
                grad_norm=grad_norm,
                seconds=duration,
                time=time.time(),
            ...
            if step % cfg["validation_every"] == 0 or (
                args.smoke_steps and step == args.smoke_steps
            ):
                val = validate(model, corpus, cfg["validation_count"])
                score = sum(val[t] / max(initial[t], 1e-8) for t in cfg["tasks"]) / len(
                    cfg["tasks"]
                )
                vr = dict(
                    step=step, losses=val, normalized_score=score, time=time.time()
                )
            ...
            if (
                time.monotonic() - last_save >= cfg["checkpoint_minutes"] * 60
                or step == 4
            ):
                save("checkpoint_latest.pt")
                last_save = time.monotonic()
            if variant == "noslots" and step == cfg["matched_preview_step"]:
                save("checkpoint_matched_early.pt")
                atomic_json(
                    run / "early_checkpoint.json",
                    dict(
                        step=step,
                        time=time.time(),
                        selection="same optimizer step as Ours early checkpoint",
                        evaluation_split="validate",
                        reference=str(args.follow / "checkpoint_early.pt"),
                    ),
                )
            if variant == "slot44" and cfg.get("early_checkpoint_time"):
                early_time = datetime.fromisoformat(
                    cfg["early_checkpoint_time"]
                ).timestamp()
                if (
                    time.time() >= early_time
                    and step % len(cfg["tasks"]) == 0
                    and not (run / "checkpoint_early.pt").exists()
                ):
                    ...
                    atomic_json(
                        ...
                        dict(
                            step=step,
                            time=time.time(),
                            selection="fixed wall-clock deadline",
                            evaluation_split="validate",
                        ),
                    )
        ...
        status(
            "interrupted" if STOP else "training_complete",
            wall_seconds=time.time() - started,
        )
        if variant == "noslots":
            reference = json.loads((args.follow / "status.json").read_text())
            atomic_json(
                run / "step_alignment.json",
                dict(
                    baseline_step=step,
                    reference_step=reference["step"],
                    reference_phase=reference["phase"],
                    equal_steps=step == reference["step"],
                    baseline_task_examples=task_examples,
                    reference_task_examples=reference["task_examples"],
                    equal_task_examples=task_examples == reference["task_examples"],
...
def main(default_variant="auto"):
    p = argparse.ArgumentParser()
    p.add_argument(
        "--variant",
        choices=["auto", "legacy", "slot44", "noslots"],
