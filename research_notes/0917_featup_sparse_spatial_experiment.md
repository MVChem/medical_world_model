...
## 09-18 上午核验：任务中断，无正式结果

09-18 09:10 核验时，调度器和两个准备进程均已不存在，GPU 无活动计算进程。
最后队列心跳为 09-17 23:18:20，最后缓存写入约为 23:18:23；两项准备日志均无
Python traceback，调度器日志为空。无法读取系统内核日志，因此尚不能确定退出原因，
不能将其归因于 OOM、重启或外部终止。

两任务的视觉特征缓存已生成：分割 4,809 例，SR 4,671 例。按记录顺序核验，
VLM 语义概率仅分别完成 125 例与 75 例；两份缓存均缺少 `complete.json`。
18 项训练全部未启动，没有正式 Dice／PSNR／SSIM 或训练后的 attention 图，
不能据此判断方法是否有效。此前短测仅验证链路。

已保留原始状态、日志和缓存，并将运行报告改为中断状态。原 8 小时窗口已结束；
此次上午核验没有启动新的 GPU 训练。恢复时需要先补齐语义缓存，采用独立进程托管，
验证任务在启动会话结束后仍持续运行；新运行需使用新目录和明确的新截止时间。
