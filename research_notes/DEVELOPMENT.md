...
整理前有独立提交和标签，整理后的改动另作提交。提交时查看暂存文件，数据、权重、运行产物和第三方仓库由 `.gitignore` 排除。当前论文源码位于 `27cvpr/`，从项目根仓库查看其修改：`git status --short -- 27cvpr`。

数据、运行产物和第三方仓库继续独立保存；提交前核对暂存清单，避免将它们纳入根仓库。
